...
vendor_number = "+917387438683"  # Vendor's phone number
...
fields_list = [
    ...
    'Offerings', 'Offering Keywords', 'Description about offerings', 'Sales Pitch',
    'LinkedIn', 'WhatsApp Number', 'Calendly Link', 'Designation', 'Role'
]
...
def save_user_details_to_excel(details):
    ...
    new_row = {
        ...
        'Description about offerings': details.get('Description about offerings', ''),
...
def whatsapp_reply():
    ...
    if current_stage == 'initial':
        if incoming_msg == 'yes':
            ...
            conversation_states[user_phone_number]['stage'] = 'requirement'
    ...
    # Requirement stage
        ...
        user_details[user_phone_number]['Requirement'] = incoming_msg.capitalize()
        conversation_states[user_phone_number]['stage'] = 'awaiting_name'
        message.body("Thanks! Please provide your name.")
        return str(response)
    
    elif current_stage == 'awaiting_name':
        user_details[user_phone_number]['Name'] = incoming_msg.capitalize()
        
        # Move to Company Name instead of Website
        next_field = get_next_field('Name')  # This will return 'Company Name'
        conversation_states[user_phone_number]['stage'] = f'awaiting_{next_field.lower().replace(" ", "_")}'
        message.body(f"Thanks {incoming_msg.capitalize()}! Please provide your {next_field}.")
        return str(response)
    ...
    for field in fields_list:
        ...
        if current_stage == 'awaiting_edit_choice':
            if incoming_msg.lower() == 'edit':
                message.body("Which field would you like to edit? Options: " + ", ".join(fields_list))
