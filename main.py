...
import os
import json
from get_vendor import handle_vendor_request,handle_client_request
...
fields_list = [
    ...
    'Offerings', 'Offering Keywords', 'Sales Pitch', 'LinkedIn', 'Email','WhatsApp Number', 'Calendly Link', 'Designation', 'Role'
]
...
def whatsapp_reply():
    ...
    if current_stage == 'initial':
        if incoming_msg == 'yes':
            ...
            message.body("*Great!* Can you write a text on what is your vendor requirement? YES , NO✍️")

        elif incoming_msg == "get client for me":
            conversation_states[user_phone_number]['stage'] = 'client_requirement'
            ...
            message.body("*Great*! Let me help you to connect with like-minded people!✍️")
            message.body(
    ...
    elif current_stage == "client_requirement":
        response_message = handle_client_request(user_phone_number,incoming_msg)
        return response_message
    ...
    if current_stage == 'awaiting_edit_choice':
        if incoming_msg.lower() == 'edit':
            ...
            if matched_vendors_str:
                first_recommendation = matched_vendors_str
